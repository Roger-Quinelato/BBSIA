...
- Entrada principal da API: `api.py` cria o app FastAPI e registra routers de RAG, admin, sistema e biblioteca.
- App/middlewares/schemas: `api_core.py` concentra configuracoes, CORS, rate limit, autenticacao opcional por `X-API-Key`, modelos Pydantic e fila de reprocessamento.
...
- Pipeline de resposta: `pipeline.py` executa busca, monta contexto, gera prompt, chama Ollama e aplica fallback extrativo em alguns cenarios.
- Geracao: `generator.py` valida endpoint/modelo Ollama e envia prompt via `/api/generate`.
- Recuperacao: `retriever.py` carrega metadados, modelo de embedding e Qdrant local, aplica filtros, busca dense no Qdrant, busca sparse BM25 local, funde via RRF, reranqueia e deduplica por `parent_id`.
- Vetor store: `vector_store.py` encapsula Qdrant local em `data/qdrant_db`, colecao `bbsia_chunks`.
- Ingestao: `/reprocessar` em `routers/admin.py` aciona `extrator_pdf_v2.run_extraction`, `chunking.run_chunking`, `embedding.run_embedding` e `reload_resources`.
- Extracao: `extrator_pdf_v2.py` le PDFs da raiz, `docs/` e `uploads/approved`; usa Docling quando disponivel, fallback PyMuPDF/OCR, e registra metadados via `classificador_artigo.py`.
- Chunking: `chunking.py` gera chunks parent-child a partir de `data/documentos_extraidos_v2.json`.
- Embeddings: `embedding.py` gera embeddings E5, recria Qdrant local e escreve `data/qdrant_index_metadata/metadata.json`.
...
- Falta um catalogo real de solucoes: `catalogo_solucoes.py` espera `catalogo/solucoes_piloto.json`, mas a pasta/arquivo nao existem.
...
- `pipeline.py` retorna resposta textual e fontes; nao retorna recomendacoes estruturadas, score de adequacao, alternativas, trade-offs ou perguntas de esclarecimento.
- `retriever.search` ranqueia chunks documentais; nao ha estrategia de ranking voltada a matching problema -> solucao.
- `query_planning.py` e uma heuristica simples, desligada por padrao (`ENABLE_QUERY_PLANNING=false`) e limitada a keywords fixas.
...
- `catalogo_solucoes.py`: transformar o catalogo de solucoes em fonte de dominio de primeira classe, com validacao, normalizacao e chunks otimizados para matching de problemas.
- `catalogo/solucoes_piloto.json`: criar/popular a fonte real de solucoes, hoje ausente.
- `embedding.py`: separar ou parametrizar melhor a indexacao de documentos e solucoes para evitar sobrescrita acidental do Qdrant oficial.
...
- `retriever.py`: adaptar busca/ranking para distinguir documentos, solucoes e possiveis tipos de resposta; incluir filtros e metadados de solucao.
- `vector_store.py`: avaliar colecoes separadas no Qdrant, por exemplo `bbsia_chunks` e `bbsia_solucoes`, ou payloads bem tipados em uma unica colecao.
- `pipeline.py`: criar uma orquestracao voltada a solucao de problemas, possivelmente com etapas de entendimento do problema, recuperacao de solucoes, recuperacao de evidencias e resposta estruturada.
- `generator.py`: ajustar prompt para recomendacao baseada em evidencias, com limites claros, alternativas e perguntas de esclarecimento quando a entrada for insuficiente.
- `faithfulness.py`: fortalecer avaliacao de grounding para recomendacoes e nao apenas respostas factuais curtas.
- `query_planning.py`: evoluir para self-query/roteamento por intencao, problema, area, maturidade e restricoes.
...
- `api_core.py`: adicionar modelos Pydantic para problema/solucao, se a API passar a aceitar payload estruturado.
...
- `README.md`: criar documentacao operacional; `pyproject.toml` aponta para `README.md`, mas o arquivo esta ausente.

## Riscos arquiteturais

- Risco de sobrescrita de indice: `embedding.run_embedding` sempre usa `data/qdrant_db`; `scripts/gerar_embeddings_solucoes.py` passa outro `index_dir`, mas isso so altera o diretorio de metadata, nao o Qdrant local. Rodar esse script pode substituir o indice principal por chunks de solucoes.
...
- Risco de acoplamento por fachadas/wildcards: `rag_engine.py` usa imports wildcard de varios modulos, e os modulos tambem possuem imports cruzados. Isso preserva compatibilidade, mas aumenta risco de regressao em refatoracoes.
...
- A proxima evolucao arquitetural deve separar claramente dois dominios: documentos de referencia e catalogo de solucoes.
- O ponto critico nao e adicionar mais LLM, e sim criar um contrato de dominio para "problema -> evidencias -> solucoes candidatas -> recomendacao".
- Antes de implementar features novas, vale corrigir a estrategia de indexacao de solucoes para nao competir com o indice documental oficial.
- A mudanca deve ser incremental e respeitar o plano existente de modularizacao: manter `api.py`, `rag_engine.py`, `retriever.search` e `pipeline.answer_question` como fachadas ate a nova camada de dominio estar coberta por testes.
